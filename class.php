...
class classweb_bypluem{
	...
	public function register($username,$password,$email,$phonenumber,$captcha){
		$key = "6LeoaSgiAAAAAOk90LXXr-CTGTycdz6rIk7wFAOm";
        $ch = curl_init();
        ...
        if ($result['success']) {
			...
			if ($stmt->RowCount() > 0) {
				...
				$inseart = $this->db->prepare("INSERT INTO `tbl_username` (`username`, `password`,`email`,`phonenumber`,`date`) VALUES (:username, :password, :email, :phonenumber,CURRENT_TIMESTAMP);");
				try {
					$inseart->execute([':username'=>$username,':password'=>$password,':email'=>$email,':phonenumber'=>$phonenumber]);
					$stmt = $this->db->prepare("SELECT * FROM tbl_username WHERE username = :username");
					$stmt->execute([':username'=>$username]);
					$result = $stmt->fetch();
					$_SESSION['id'] = $result['id'];
	...
	public function show_card_script(){
		$stmt = $this->db->prepare("SELECT * FROM card_script");
		$stmt->execute();
		$result = $stmt->fetchAll();
		return $result;
	}
	public function totalscript($id_card){
		$stmt = $this->db->prepare("SELECT count(id) as total FROM stock_script WHERE id_card = :id_card AND username_buy = '0'");
		$stmt->execute(['id_card'=>$id_card]);
		$result = $stmt->fetch();
		return $result;
	}
	public function allscript(){
		$stmt = $this->db->prepare("SELECT count(id) as total FROM stock_script");
		$stmt->execute();
		$result = $stmt->fetch();
		return $result;
	}
	public function soldscript(){
		$stmt = $this->db->prepare("SELECT count(id) as total FROM stock_script WHERE username_buy != '0'");
		$stmt->execute();
		$result = $stmt->fetch();
		return $result;
	}
	...
	public function buyscript($id_card){
		$resultuser = $this->resultuser();
		$card = $this->db->prepare("SELECT * FROM card_script WHERE id = :id");
        $card->execute([':id'=>$id_card]);
        $resultcard = $card->fetch();

		$stmtstock = $this->db->prepare("SELECT count(id) as total FROM stock_script WHERE id_card = :id_card AND username_buy = '0'");
		$stmtstock->execute(['id_card'=>$id_card]);
		$resultstock = $stmtstock->fetch();
		if($resultstock['total'] == 0){
			echo json_encode(array('status'=>"error",'message'=>"สินค้าหมด"));
			...
			if($resultuser['point'] < $resultcard['price']){
				...
				$stmt = $this->db->prepare("SELECT * FROM stock_script WHERE id_card = :id_card AND username_buy = '0' LIMIT 1");
				$stmt->execute([':id_card'=>$id_card]);
				$result = $stmt->fetch();
				$updatepoint = $this->db->prepare("UPDATE `tbl_username` SET `point`= point - :point WHERE id = :id");
				$history_shop = $this->db->prepare("INSERT INTO `history_shop` (`username`, `name`, `details`, `price`, `date`) VALUES (:username, :name, :details, :price, CURRENT_TIMESTAMP);");
				$updatestock = $this->db->prepare("UPDATE `stock_script` SET `username_buy`= :username_buy WHERE id = :id");
				try {
					$updatepoint->execute([':point'=>$resultcard['price'],':id'=>$_SESSION['id']]);
					$history_shop->execute([':username'=>$_SESSION['username'],':name'=>$resultcard['name'],':details'=>$result['details'],':price'=>$resultcard['price']]);
					$updatestock->execute([':username_buy'=>$_SESSION['username'],':id'=>$result['id']]);
	...
	public function show_card_scriptlimit(){
		$stmt = $this->db->prepare("SELECT * FROM card_script ORDER BY id DESC LIMIT 3");
		$stmt->execute();
		$result = $stmt->fetchAll();
		return $result;
	}
	...
	public function totaltopup(){
		$stmt = $this->db->prepare("SELECT sum(amount) as total FROM history_topup WHERE username = :username");
		$stmt->execute([':username'=>$_SESSION['username']]);
		$result = $stmt->fetch();
		return $result;
	}
	public function stock_product(){
		$stmt = $this->db->prepare("SELECT * FROM stock_product");
		$stmt->execute();
		$result = $stmt->fetchAll();
		return $result;
	}
	public function game_new(){
		$stmt = $this->db->prepare("SELECT * FROM game_new");
		$stmt->execute();
		$result = $stmt->fetchAll();
		return $result;
	}
...
class classadmin_bypluem{
	...
	public function edit_stock_product($name,$list,$image,$id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$edit_stock_product = $this->db->prepare("UPDATE `stock_product` SET `name`= :name,`list`= :list,`image`= :image WHERE id = :id");
			try {
				$edit_stock_product->execute([':name'=>$name,':list'=>$list,':image'=>$image,':id'=>$id]);
	...
	public function show_stock_product_id($id){
		$stmt = $this->db->prepare("SELECT * FROM stock_product WHERE id = :id");
		$stmt->execute([':id'=>$id]);
		$result = $stmt->fetch();
		return $result;
	}
	public function del_stock_product($id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$stock_product = $this->db->prepare("DELETE FROM `stock_product` WHERE id = :id");
			$stock_product->execute([':id'=>$id]);
			echo json_encode(array('status'=>"success",'message'=>"ลบสินค้าสำเร็จ"));
		}
	}
	public function add_stock_product($name,$list,$image){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$inseart = $this->db->prepare("INSERT INTO `stock_product` (`name`,`list`,`image`) VALUES (:name,:list,:image)");
			try {
				$inseart->execute([':name'=>$name,':list'=>$list,':image'=>$image]);
	...
	public function show_stock_product(){
		$stmt = $this->db->prepare("SELECT * FROM stock_product");
		$stmt->execute();
		$result = $stmt->fetchAll();
		return $result;
	}
	public function edit_game_new($name,$details,$image,$id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$edit_game_new = $this->db->prepare("UPDATE `game_new` SET `name`= :name,`details`= :details,`image`= :image WHERE id = :id");
			try {
				$edit_game_new->execute([':name'=>$name,':details'=>$details,':image'=>$image,':id'=>$id]);
				echo json_encode(array('status'=>"success",'message'=>"บันทึกข้อมูลสำเร็จ!"));
			} catch (Exception $e) {
				echo json_encode(array('status'=>"error",'message'=>$e->getmessage()));
			}
	...
	public function show_game_new_id($id){
		$stmt = $this->db->prepare("SELECT * FROM game_new WHERE id = :id");
		$stmt->execute([':id'=>$id]);
		$result = $stmt->fetch();
		return $result;
	}
	public function del_game_new($id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$game_new = $this->db->prepare("DELETE FROM `game_new` WHERE id = :id");
			$game_new->execute([':id'=>$id]);
			echo json_encode(array('status'=>"success",'message'=>"ลบสินค้าสำเร็จ"));
		}
	}
	public function add_game_new($name,$details,$image){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$inseart = $this->db->prepare("INSERT INTO `game_new` (`name`,`details`,`image`) VALUES (:name,:details,:image)");
			try {
				$inseart->execute([':name'=>$name,':details'=>$details,':image'=>$image]);
				echo json_encode(array('status'=>"success",'message'=>"เพิ่มข้อมูลสำเร็จ"));
			} catch (Exception $e) {
				echo json_encode(array('status'=>"error",'message'=>$e->getmessage()));
			}
	...
	public function del_script($id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$del_script = $this->db->prepare("DELETE FROM `card_script` WHERE id = :id");
			$del_script->execute([':id'=>$id]);
			echo json_encode(array('status'=>"success",'message'=>"ลบสินค้าสำเร็จ"));
		}
	}
	public function add_script($name,$image,$price,$details){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$inseart = $this->db->prepare("INSERT INTO `card_script` (`name`, `image`, `price`, `details`) VALUES (:name, :image, :price, :details);");
			try {
				$inseart->execute([':name'=>$name,':image'=>$image,':price'=>$price,':details'=>$details]);
				echo json_encode(array('status'=>"success",'message'=>"เพิ่มข้อมูลสำเร็จ"));
			} catch (Exception $e) {
	...
	public function show_script_id($id){
		$stmt = $this->db->prepare("SELECT * FROM card_script WHERE id = :id");
		$stmt->execute([':id'=>$id]);
		$result = $stmt->fetch();
		return $result;
	}
	public function edit_script($name,$image,$price,$details,$id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$edit_script = $this->db->prepare("UPDATE `card_script` SET `name`= :name, image = :image, price = :price, details = :details WHERE id = :id");
			try {
				$edit_script->execute([':name'=>$name,':image'=>$image,':price'=>$price,':details'=>$details,':id'=>$id]);
				echo json_encode(array('status'=>"success",'message'=>"บันทึกข้อมูลสำเร็จ!"));
			} catch (Exception $e) {
				echo json_encode(array('status'=>"error",'message'=>$e->getmessage()));
			}
	...
	public function add_stock_script($details,$id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			...
			$exp_lines = explode("\n", $string);
			foreach ($exp_lines as $each_line) {
				$inseart = $this->db->prepare("INSERT INTO `stock_script` (`details`, `id_card`) VALUES (:details, :id_card);");
				$inseart->execute([':details'=>$each_line,':id_card'=>$id]);
			}
			echo json_encode(array('status'=>"success",'message'=>"เพิ่มข้อมูลสำเร็จ"));
		}
	}
	public function stock_script_id($id){
		$stmt = $this->db->prepare("SELECT * FROM stock_script WHERE id_card = :id_card AND username_buy = '0'");
		$stmt->execute([':id_card'=>$id]);
		$result = $stmt->fetchAll();
		return $result;
	}
	public function del_stock_script($id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$del_script = $this->db->prepare("DELETE FROM `stock_script` WHERE id = :id");
			$del_script->execute([':id'=>$id]);
			echo json_encode(array('status'=>"success",'message'=>"ลบสินค้าสำเร็จ"));
		}
	}
	...
	public function show_edit_stock_script($id){
		$stmt = $this->db->prepare("SELECT * FROM stock_item WHERE id = :id");
		$stmt->execute([':id'=>$id]);
		$result = $stmt->fetch();
		return $result;
	}
	public function edit_stock_script($details,$price,$image,$id){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
	...
	public function settings_web($title,$logo,$phone,$contact,$banner,$info,$page,$clip){
		$resultuser = $this->resultuser();
		if($resultuser['class'] != "1"){
			echo json_encode(array('status'=>"error",'message'=>"ไม่ใช่แอดมิน"));
		}else{
			$settings_web = $this->db->prepare("UPDATE `web_config` SET `title`= :title, logo = :logo, phone = :phone, contact = :contact, banner = :banner, info = :info, page = :page, clip = :clip WHERE id = '1'");
			try {
				$settings_web->execute([':title'=>$title,':logo'=>$logo,':phone'=>$phone,':contact'=>$contact,':banner'=>$banner,':info'=>$info,':page'=>$page,':clip'=>$clip]);
				echo json_encode(array('status'=>"success",'message'=>"บันทึกข้อมูลสำเร็จ!"));
			} catch (Exception $e) {
				echo json_encode(array('status'=>"error",'message'=>$e->getmessage()));
			}
